...
**药物作用机理解释流水线**的核心目标是从医学文本中抽取结构化知识，并围绕药物、疾病、基因等医学实体构建可用于下游分析的高质量三元组数据。该流水线首先加载医学本体作为约束，然后进行医学三元组抽取，再按照目标本体筛选关键关系，最后基于筛选后的医学证据路径生成药物作用机制解释。

我们支持以下应用场景：

- 从医学文献、临床报告或药物说明文本中抽取结构化三元组
- 围绕药物、疾病、基因等实体构建药物作用机理解释
