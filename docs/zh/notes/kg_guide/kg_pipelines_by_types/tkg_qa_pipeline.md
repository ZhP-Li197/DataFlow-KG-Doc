---
title: 时序知识图谱问答流水线 (TKG QA Pipeline)
...
**时序知识图谱问答流水线 (TKGQA_APIPipeline)** 面向“从自然语言文本中提取带有时间属性的知识结构，并基于特定时间范围和逻辑路径生成时序问答对”的场景。与常规知识图谱不同，时序知识图谱（Temporal Knowledge Graph, TKG）强调事实的有效时间或发生时间。该流水线能够自动提取时序事实、按时间窗口过滤、构建多跳关系路径，并最终生成与评估高质量的时间点问答对。
