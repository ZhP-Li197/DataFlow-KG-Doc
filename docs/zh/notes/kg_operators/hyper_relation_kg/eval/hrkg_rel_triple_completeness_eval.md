...
permalink: /zh/kg_operators/hyper_relation_kg/eval/hrkg_rel_triple_completeness_eval/
---

## 📚 概述

[HRKGTripleCompletenessEvaluator](https://github.com/ZhP-Li197/DataFlow-KG/tree/main/dataflow/operators/hyper_relation_kg/eval/hrkg_rel_triple_completeness_eval.py) 是一个基于大语言模型（LLM）的超关系知识图谱三元组完整性评估算子。它评估每个超关系元组是否包含所有必要信息（主语、宾语、关系和关键属性），并为每个元组分配完整性分数（0-1）。算子支持 Python 列表和 JSON 编码的字符串列表输入，并返回与输入元组对齐的分数列表。
...
| 参数名 | 类型 | 默认值 | 说明 |
| :-- | :-- | :-- | :-- |
| **llm_serving** | LLMServingABC | 必填 | 大语言模型服务实例，用于完整性评估推理。 |
...
`run` 从 `storage` 读取 DataFrame，验证存储可用，并遍历每一行。对于每一行，从 `input_key` 提取元组列表，解析它（支持 Python 列表和 JSON 字符串格式），然后使用 Prompt 模板调用 LLM 评估完整性分数。结果分数列表写入 `output_key` 列。如果某行为空、解析失败或 LLM 调用抛出异常，则该行写入空列表。函数返回包含 `output_key` 字符串的列表。
...
| **output_key** | str | "completeness_scores" | 输出列名，对应完整性分数列表字段。 |
...
| **completeness_scores** | **List[float]** | 每个元组的完整性分数（0-1）；1=完全完整，0=严重不完整。 |
