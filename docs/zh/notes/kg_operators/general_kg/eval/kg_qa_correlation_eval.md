...
permalink: /zh/kg_operators/general_kg/eval/kg_qa_correlation_eval/
---

## 📚 概述
`KGQACorrelationEvaluator` 是一个用于评估知识图谱 QA 对相关性的评估类算子。
它会读取每一行中的 QA 问答对列表，调用大模型对这些 QA 对的相关性进行打分，并将评分结果写回 DataFrame。

这个算子的几个关键特点如下：

- 依赖 `LLMServingABC` 提供大模型评估能力
- 默认使用 `KGQACorrelationPrompt` 构造提示词
- 默认读取 `QA_pairs` 列，默认输出到 `correlation_scores`
...
| 参数名 | 类型 | 默认值 | 说明 |
| :-- | :-- | :-- | :-- |
| `llm_serving` | `LLMServingABC` | - | 大模型服务对象。算子通过 `generate_from_input` 对 QA 对的相关性进行评分。 |
| `lang` | `str` | `"en"` | Prompt 语言。构造函数会创建 `KGQACorrelationPrompt(lang)`。 |
...
`run` 会先从 `storage` 中读取 DataFrame，并逐行取出 `input_key` 指定的列，重新组织成内部统一使用的 `{"QA_pairs": ...}` 结构。随后算子调用 `process_batch()` 对每一行进行评估：若单元格内容是字符串，会先尝试按 JSON 解析；若为空或解析失败，则该行输出空列表。对于有效 QA 对列表，算子会构造系统提示词和用户提示词，要求模型仅返回包含 `correlation_scores` 的 JSON。最终结果会写回 `output_key` 指定的列。
...
| `output_key` | `str` | `"correlation_scores"` | 输出评分列名，用于保存每个 QA 对对应的相关性分数列表。 |
...
| `correlation_scores` | `List[float]` | 与输入 QA 对按位置对齐的相关性评分列表。 |
...
#### 相关链接
- 算子实现：`DataFlow-KG/dataflow/operators/general_kg/eval/kg_qa_correlation_eval.py`
