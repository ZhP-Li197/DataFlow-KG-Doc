---
title: KGQANaturalFilter
createTime: 2026/04/10 15:40:44
permalink: /zh/kg_operators/general_kg/filter/kg_qa_natural_filtering/
---

## 📚 概述
`KGQANaturalFilter` 是一个用于根据自然度得分过滤 QA 对的过滤类算子。
它会读取每一行中的 QA 对列表及对应的自然度得分列表，按位置对齐后保留得分在指定范围内的 QA 对，并将过滤结果写回 DataFrame。
...
- 通常配合 `KGQANaturalEvaluator` 的输出列使用
...
| `score_key` | `str` | `"naturalness_scores"` | 输入自然度得分列名，通常为 `KGQANaturalEvaluator` 的输出列。每个单元格应为与 `input_key` 等长的 `List[float]`。 |
...
## 🤖 示例用法
```python
from dataflow.operators.general_kg.filter.kg_qa_natural_filtering import (
    KGQANaturalFilter,
)

operator = KGQANaturalFilter(merge_to_input=False)
operator.run(
    storage=storage,
    input_key="QA_pairs",
    score_key="naturalness_scores",
...
| `naturalness_scores` | `List[float]` | 各 QA 对对应的自然度得分，由上游评估算子生成。 |
...
#### 相关链接
- 算子实现：`DataFlow-KG/dataflow/operators/general_kg/filter/kg_qa_natural_filtering.py`
- 上游评估算子：`DataFlow-KG/dataflow/operators/general_kg/eval/kg_qa_natural_eval.py`