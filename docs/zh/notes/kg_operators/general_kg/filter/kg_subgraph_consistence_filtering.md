...
permalink: /zh/kg_operators/general_kg/filter/kg_subgraph_consistence_filtering/
---

## 📚 概述
`KGSubgraphConsistenceFilter` 是一个用于根据一致性得分筛选知识图谱子图的过滤类算子。
它会读取子图列和对应的一致性得分列，对整行进行阈值过滤，保留得分在指定范围内的行并写回 DataFrame。
...
- 通常配合 `KGSubgraphConsistency` 输出的 `consistency_score` 列使用
...
| `output_key` | `str` | `"consistency_score"` | 用于过滤判断的一致性得分列名，通常为 `KGSubgraphConsistency` 的输出列。 |
| `min_score` | `float` | `0.95` | 保留行的最低得分阈值（含）。 |
...
| `consistency_score` | `float` | 用于过滤判断的一致性得分（原样保留）。 |
...
#### 相关链接
- 算子实现：`DataFlow-KG/dataflow/operators/general_kg/filter/kg_subgraph_consistence_filtering.py`
- 上游评估算子：`DataFlow-KG/dataflow/operators/general_kg/eval/kg_subgraph_consistence_eval.py`