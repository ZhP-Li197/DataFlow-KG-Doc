...
permalink: /zh/kg_operators/general_kg/filter/kg_subgraph_scale_filtering/
---

## 📚 概述
`KGSubgraphScaleFilter` 是一个用于根据规模指标筛选知识图谱子图的过滤类算子。
它会读取子图列和对应的规模得分列，对整行进行阈值过滤，保留得分在指定范围内的行并写回 DataFrame。
...
- 通常配合 `KGSubgraphScaleEvaluator` 输出的 `density`、`num_nodes`、`num_edges` 等列使用
...
> 注意：`output_key` 在本算子中语义为**用于过滤判断的得分列名**，而非新增的输出列名。算子不新增任何列。
...
| `output_key` | `str` | `"density"` | 用于过滤判断的规模得分列名，通常为 `KGSubgraphScaleEvaluator` 的输出列之一（如 `density`、`num_nodes`、`num_edges`）。 |
...
| `density`（或其他得分列） | `float` | 用于过滤判断的规模得分（原样保留）。 |
...
#### 相关链接
- 算子实现：`DataFlow-KG/dataflow/operators/general_kg/filter/kg_subgraph_scale_filtering.py`
- 上游评估算子：`DataFlow-KG/dataflow/operators/general_kg/eval/kg_subgraph_scale_eval.py`