---
title: KGSubgraphScaleFilter
createTime: 2026/04/10 15:40:44
permalink: /en/kg_operators/general_kg/filter/kg_subgraph_scale_filtering/
---

## 📚 Overview
`KGSubgraphScaleFilter` is a filtering operator that filters knowledge graph subgraphs based on scale metrics.
It reads the subgraph column and the corresponding scale score column, applies threshold-based filtering at the row level, retains rows whose scores fall within the specified range, and writes the results back to the DataFrame.
...
- Typically used with scale metric columns output by `KGSubgraphScaleEvaluator`, such as `density`, `num_nodes`, and `num_edges`
...
| `output_key` | `str` | `"density"` | Scale score column name used for filtering, typically one of the output columns from `KGSubgraphScaleEvaluator` (e.g., `density`, `num_nodes`, `num_edges`). |
...
operator = KGSubgraphScaleFilter()
operator.run(
    storage=storage,
    input_key="subgraph",
    output_key="density",
...
| `density` (or other score columns) | `float` | Scale score used for filtering (preserved as-is). |
...
```json
[
  {"subgraph": ["..."], "density": 0.5},
  {"subgraph": ["..."], "density": 0.1},
  {"subgraph": ["..."], "density": 0.8}
]
```

#### Example Output (`min_score=0.3`)
```json
[
  {"subgraph": ["..."], "density": 0.5},
  {"subgraph": ["..."], "density": 0.8}
...
#### Related Links
- Operator implementation: `DataFlow-KG/dataflow/operators/general_kg/filter/kg_subgraph_scale_filtering.py`
- Upstream evaluation operator: `DataFlow-KG/dataflow/operators/general_kg/eval/kg_subgraph_scale_eval.py`