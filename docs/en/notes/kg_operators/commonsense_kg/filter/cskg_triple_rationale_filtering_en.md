...
## 📚 Overview
`CSKGTripleRationaleFilter` is a filter-category operator for filtering commonsense triples by rationale-related scores.
It reads a triple list together with the corresponding score list, keeps only the triples whose scores fall inside the configured range, and writes the filtered result back into the DataFrame.
...
- It reads `triple` and `rationale_scores` by default
- It writes to `filtered_triple` by default
...
| `score_key` | `str` | `"rationale_scores"` | Input rationale-score column name. |
...
| `rationale_scores` | `List[float]` | Rationale-score list aligned positionally with the triples. |
