...
permalink: /en/kg_operators/hyper_relation_kg/filter/hrkg_rel_triple_completeness_filtering/
---

## 📚 Overview

[HRKGTripleCompletenessFilter](https://github.com/ZhP-Li197/DataFlow-KG/tree/main/dataflow/operators/hyper_relation_kg/filter/hrkg_rel_triple_completeness_filtering.py) is a hyper-relational KG triple filtering operator based on completeness scores. It filters tuples whose completeness scores fall within a user-specified range. The operator aligns triples and scores by position and retains only those satisfying `min_score <= score <= max_score`. The filtered results can be written to a new column or overwrite the input column via parameters for flexible integration into different pipelines.
