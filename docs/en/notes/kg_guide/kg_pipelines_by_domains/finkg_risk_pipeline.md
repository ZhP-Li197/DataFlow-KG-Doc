...
- Pipeline script: `api_pipelines/finkg_risk_pipeline.py`
- Default data: `example_data/FinKGRiskPipeline/input.json`

The default data contains at least:
...
The default model is `gpt-4o-mini`. You can override the defaults with `DF_API_URL`, `DF_LLM_MODEL`, or `DF_FINKG_INPUT_FILE`.
...
The following is a reference view of the `FinKGRiskPipeline` code generated by `dfkg init`. For execution, use the generated `api_pipelines/finkg_risk_pipeline.py` file.
