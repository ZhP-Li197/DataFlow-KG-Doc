...
- Pipeline script: `api_pipelines/graph_rag_pipeline.py`
- Default data: `example_data/GraphRAGPipeline/input.json`
...
The default model is `gpt-4o-mini`. You can override the defaults with `DF_API_URL`, `DF_LLM_MODEL`, or `DF_GRAPHRAG_INPUT_FILE`.
...
The following is a reference view of the `GraphRAGPipeline` code generated by `dfkg init`. For execution, use the generated `api_pipelines/graph_rag_pipeline.py` file.
